...
class ArkAssets:
    ...
    def get_version(server: Servers = Servers.OFFICAL) -> tuple[str, str]:
        js = requests.get('https://ak-conf.hypergryph.com/config/prod/{}/Android/version'.format(
            'offical' if server == ArkAssets.Servers.OFFICAL else 'b')).json()
    ...
    def get_hot_update_list(self) -> tuple[dict, int, int]:
        js = requests.get('https://ak.hycdn.cn/assetbundle/{}/Android/assets/{}/hot_update_list.json'.format(
            'offical' if self.server == ArkAssets.Servers.OFFICAL else 'bilibili',
    ...
    def download_fromlist(self, keys: list[str], savedir: str, threading_count: int = 4):
        ...
        with tqdm(total=_count, desc='\033[1;33m总进度 已下载0B\033[m', unit='个', position=threading_count, leave=False) as pbar,\
            ...
            def down(files, n):
                ...
                while(len(list(files.keys())) > 0):
                    ...
                    _read.release()
                    try:
                        stream = self.download_asset(file, lock=_lock, bar_position=n, thread_num=n)
                        _size.acquire()
                        res_size += len(stream)
                        _lock.acquire()
                        ...
                        def unzip(stream, name, md5, path):
                                            ...
                                            with open(str(_p / (data.name + '.obj')), "wt", newline = "") as f:
                                                ...
                                                if font.m_FontData:
                                                    ...
                                                    if font.m_FontData[0:4] == b"OTTO":
                                                        extension = ".otf"
                                                    _p.mkdir(parents=True, exist_ok=True)
                                                    with open(str(_p / (font.name + extension)), "wb") as f:
                            ...
                            threading.Thread(target=unpack, args=(Path(savedir) / name,), daemon=True).start()
    ...
    def download_asset(self, path: str, bar_position: int = 0, lock: threading.Lock = None, thread_num: int = 0) -> bytes:
        global pos
...
ArkAssets(True).download('/storage/emulated/0/assets')